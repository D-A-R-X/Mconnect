...
class CompletedVisitDetailFragment : Fragment() {
    ...
    private fun bindContent(root: View) {
        ...
        // ---- Status summary ----
        // When the trip ended without an outcome (sheet dismissed, app
        // crash, OS kill), the visit is in limbo: status=completed on the
        // server but no decision was recorded. Surface this clearly and
        // hand the user a "Record Outcome" CTA that reopens the bottom
        // sheet — anything else strands the visit forever.
        val outcomePending = outcome.isNullOrBlank()
        ...
        // Status pill flips amber when outcome is pending so the
        // top-of-screen badge matches the CP-list card colouring.
